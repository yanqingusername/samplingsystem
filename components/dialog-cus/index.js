...
Component({
  ...
  methods: {
    ...
    dialogBoxnumCancel() {
      ...
      this.triggerEvent('dialogBoxnumCancel', '')
    },
    dialogBoxnumSure() {
      if(this.data.box_num){
        this.setData({
          showDialog: false
        })
        this.triggerEvent('dialogBoxnumSure', this.data.box_num)
      }else{
        box.showToast("请输入箱码");
      }
    },
    boxnumInput: function (e) {
      if (e) {
        var str = e.detail.value;
        // str = utils.checkInput(str);
        this.setData({
